//
// Created by wlt on 19-4-27.
//

#ifndef PROJECT_VOXEL_GRID_KEEP_PROPERTY_H
#define PROJECT_VOXEL_GRID_KEEP_PROPERTY_H
...
//! Modified PCL classes
namespace pcl {

/**
  * @brief Modified VoxelGridFilter which can keep other fields in an voxel cell.
  * Fields are inherit from randomly selected point from voxel cell.
  * @tparam PointT Point Type
  */
    template<typename PointT>
    class VoxelGridKeepFields : public Filter<PointT> {
    protected:
    ...
    public:
        typedef boost::shared_ptr<VoxelGridKeepFields<PointT> > Ptr;
        typedef boost::shared_ptr<const VoxelGridKeepFields<PointT> > ConstPtr;
        ...
        VoxelGridKeepFields() :
            ...
            filter_name_ = "VoxelGridKeepFields";
        }

        /** \brief Destructor. */
        virtual ~VoxelGridKeepFields() {
        }
        ...
        */
        ...
        getCentroidIndex(const PointT &p) {
            return (leaf_layout_.at((Eigen::Vector4i(static_cast<int> (floor(p.x * inverse_leaf_size_[0])),
                                                     ...
                                                     static_cast<int> (floor(p.z * inverse_leaf_size_[2])), 0) - min_b_).dot(
                    divb_mul_)));
        }
    ...
    protected:
        ...
        applyFilter(PointCloud &output) {
            ...
            if ((dx * dy * dz) > static_cast<int64_t>(std::numeric_limits<int32_t>::max())) {
                PCL_WARN("[pcl::%s::applyFilter] Leaf size is too small for the input dataset. Integer indices would overflow.max-min %f,%f,%f,%f,%f,%f\n",
                         getClassName().c_str(),min_p[0],min_p[1],min_p[2],max_p[0],max_p[1],max_p[2]);
            ...
            if (!filter_field_name_.empty()) {
                ...
                for (std::vector<int>::const_iterator it = indices_->begin(); it != indices_->end(); ++it) {
                    ...
                    int ijk0 =
                            static_cast<int> (floor(input_->points[*it].x * inverse_leaf_size_[0]) - static_cast<float> (min_b_[0]));
                    int ijk1 =
                            static_cast<int> (floor(input_->points[*it].y * inverse_leaf_size_[1]) - static_cast<float> (min_b_[1]));
                    int ijk2 =
                            static_cast<int> (floor(input_->points[*it].z * inverse_leaf_size_[2]) - static_cast<float> (min_b_[2]));
            ...
            if (save_leaf_layout_) {
                try {
                    ...
                    uint32_t reinit_size =
                            std::min(static_cast<unsigned int> (new_layout_size), static_cast<unsigned int> (leaf_layout_.size()));
...
#endif //PROJECT_VOXEL_GRID_KEEP_PROPERTY_H
